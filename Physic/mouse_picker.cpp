...
mouse_picker::mouse_picker(
	...
	collider_resolver(collider_resolver), drawer(drawer)
{
}
...
void mouse_picker::take_object()
{
	for (auto& physic_object : physic_objects) {
		if (std::any_of(physic_object->convex_shapes.cbegin(), physic_object->convex_shapes.cend(),
			[&](auto& convex_shape){ return gjk.contains_point(convex_shape.vertices, position);}))
		{
			selected = physic_object;
			shoulder = (position - physic_object->position).rotate(-selected->radians);
			break;
		}
	}
}
...
void mouse_picker::update_object()
{
	...
	vector2 velocity = position - new_shoulder;
...
void mouse_picker::control_object(sf::Event event) 
{
	...
	case sf::Event::MouseMoved:
		position = mouse_position(event.mouseMove);
		break;
...
{
	return vector2(
		mouseMoveEvent.x - (global::screen_width / 2),
